...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // 设置产品特色状态（置顶/取消置顶）
  app.patch("/api/products/:id/feature", requireAdmin, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      ...
      if (typeof featured !== 'boolean') {
        return res.status(400).json({ message: "featured值必须是布尔类型" });
      }
      
      const product = await storage.setProductFeatured(id, featured);
      ...
      return res.json({
        message: featured ? "产品已设为特色/置顶" : "产品已取消特色/置顶",
        product
      });
    } catch (error) {
      console.error('设置产品特色状态时出错:', error);
      res.status(500).json({
        message: "设置产品特色状态时出错",
        error: error instanceof Error ? error.message : String(error)
      });
  ...
  // 重新排序产品
  app.patch("/api/products/:id/reorder", requireAdmin, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      ...
      if (direction !== 'up' && direction !== 'down') {
        return res.status(400).json({ message: "direction必须是'up'或'down'" });
      }
      ...
      let targetOrder = currentProduct.display_order || 0;
      
      if (direction === 'up' && currentIndex > 0) {
        // 将当前产品的显示顺序设置为上一个产品的显示顺序+1
        const prevProduct = sortedProducts[currentIndex - 1];
        targetOrder = (prevProduct.display_order || 0) + 1;
      } else if (direction === 'down' && currentIndex < sortedProducts.length - 1) {
        // 将当前产品的显示顺序设置为下一个产品的显示顺序-1
        const nextProduct = sortedProducts[currentIndex + 1];
        targetOrder = Math.max(0, (nextProduct.display_order || 0) - 1);
      } else {
        return res.status(400).json({ 
          message: `无法${direction === 'up' ? '上移' : '下移'}产品，已到达列表边界` 
        });
      }
      
      // 更新产品显示顺序
      const updatedProduct = await storage.updateProductOrder(id, targetOrder);
      
      if (!updatedProduct) {
        return res.status(500).json({ message: "更新产品顺序失败" });
      }
